...
void solve()
{
	...
	m.clear();
	int lft = 0, rht = 0, cnt = 0;
	auto nextStep = [&](){
		m[a[rht]]++;
		if(a[rht] <= mx and m[a[rht]] == 1)
			cnt++;
		while(lft < rht and cnt == mx and (m[a[lft]] > 1 or a[lft] > mx)){
			m[a[lft]]--;
			lft++;
		}
	};
	...
	rep(i, n){
		nextStep();
		if(cnt == mx){
			(prefix[rht] += 1 + (lft ? prefix2[lft - 1] : 0)) % mod;
		}
		prefix2[rht] = prefix[rht];
		if(rht){
			(prefix2[rht] += prefix2[rht - 1]) % mod;
		}
		prefix[rht] %= mod;
		prefix2[rht] %= mod;
		dbg(rht, lft, cnt);
